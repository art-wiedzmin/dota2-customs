...
/** 人机难度：0=关（不补机器人），1 简单，2 困难，3 令人发狂。与服务端 Boot.Config.bot_difficulty 一致 */
var g_BotDifficulty = 0;
...
function RefreshBotDifficultyButtons() {
	var e = $("#BotDiffEasy");
	var h = $("#BotDiffHard");
	var i = $("#BotDiffInsane");
	if (!e || !h || !i) {
	...
	e.SetHasClass("BotDiffOn", g_BotDifficulty === 1);
	e.SetHasClass("BotDiffOff", g_BotDifficulty !== 1);
	h.SetHasClass("BotDiffOn", g_BotDifficulty === 2);
	h.SetHasClass("BotDiffOff", g_BotDifficulty !== 2);
	i.SetHasClass("BotDiffOn", g_BotDifficulty === 3);
	i.SetHasClass("BotDiffOff", g_BotDifficulty !== 3);
}

/** @param {number} level 1 easy 2 hard 3 insane；再次点击已选中的档可关闭人机（0） */
function OnBotDifficultyPressed(level) {
	...
	if (g_BotDifficulty === lv) {
		g_BotDifficulty = 0;
	} else {
		g_BotDifficulty = lv;
	}
	RefreshBotDifficultyButtons();
	GameEvents.SendCustomGameEventToServer("Lua_Team", {
		data: {
			tp: "set_bot_difficulty",
			difficulty: g_BotDifficulty
		}
	});
}

function OnBotConfigChanged(data) {
	if (!data) {
		return;
	}
	if (data.difficulty !== undefined && data.difficulty !== null) {
		g_BotDifficulty = parseInt(data.difficulty, 10);
		if (isNaN(g_BotDifficulty)) {
			g_BotDifficulty = 0;
		}
	} else if (data.enabled !== undefined) {
		var en = data.enabled == 1 || data.enabled === true || data.enabled === "1";
		g_BotDifficulty = en ? 2 : 0;
	}
	RefreshBotDifficultyButtons();
}
...
function FindOrCreatePanelForPlayer(playerId, parent) {
	...
	newPlayerPanel.BLoadLayout("file://{resources}/layout/custom_game/ingame/Team/team_select_player.xml", false, false);
...
function UpdateTeamPanel(teamPanel) {
	...
	for (var i = teamPlayers.length; i < nNumPlayerSlots; ++i) {
		...
		if (playerSlot.GetChildCount() == 0) {
			...
			empty_slot.BLoadLayout("file://{resources}/layout/custom_game/ingame/Team/team_select_empty_slot.xml", false, false);
		}
	}
...
(function () {
	...
	for (var teamId of allTeamIDs) {
		...
		teamNode.BLoadLayout("file://{resources}/layout/custom_game/ingame/Team/team_select_team.xml", false, false);
	...
	GameEvents.Subscribe("UI_TeamBotConfig", OnBotConfigChanged);
