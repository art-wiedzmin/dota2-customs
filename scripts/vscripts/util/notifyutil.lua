...
--@param #table keyValue 格式化国际化用的参数。 可为空。 如果有要再次进行国际化的键值，则key加上前缀"local_"
...
--比如国际化为 “公共仓库有：{s:name}<Panel>”，则图片就会出现在<Panel>所在的位置，<Panel>位置在哪里都行。 目前仅支持显示一个物品
--@param #table keyValue 如果text中含有格式化字符串（{s:xxx}，仅支持字符串），则传入这个值即可，key就是对应的key，value是显示值
function m.ShowLifeMsg(PlayerID,textGroup,pid,itemNameOrID,keyValue)
	if textGroup then
		if type(textGroup) == "string" then
			textGroup = {textGroup}
		end
		...
		if PlayerID then
			SendToClient(PlayerID,"show_custom_sys_msg",{msg=textGroup,player=pid,item=itemNameOrID,fmtKV=keyValue,type = 'life'})
		else
			SendToAllClient("show_custom_sys_msg",{msg=textGroup,player=pid,item=itemNameOrID,fmtKV=keyValue,type = 'life'})
		end
	end
end
