...
function EntityNotNull(entity)
	--IsNull和IsValidEntity()有区别，比如modifier的IsNull是有动态有效的，但是IsValidEntity只会返回false。IsValidEntity所认定的最顶级基类，不确定是哪个。一般就不用了
	return type(entity) == "table" and not entity:IsNull()
end

---判断一个实体是否为空：lua对象为空，或对应的c++实体为空
--@param #table entity 实体对象
function EntityIsNull(entity)
	return not EntityNotNull(entity)
end
...
function m.remove(entity)
	if EntityNotNull(entity) then
		...
		if entity.ForceKill then --此方法是BaseNPC的方法
			m.kill(entity,true)
		else
			entity:RemoveSelf();
		end
